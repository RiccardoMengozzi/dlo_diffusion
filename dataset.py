...
def prepare_obs_1_action_full(dlo_0_n, dlo_1_n, action_n, pred_h_dim):

    action_steps = action_n[1:] / pred_h_dim
    actions_idx = np.tile(action_n[0], (pred_h_dim, 1))
    actions = np.tile(action_steps, (pred_h_dim, 1))
    actions = np.concatenate([actions_idx, actions], axis=1)
...
def load_and_process_sample(file, disp_range=None, rot_range=None, pred_h_dim=16):
    ...
    norm_obs, actions = prepare_obs_1_action_full(dlo_0_n, dlo_1_n, action_n, pred_h_dim=pred_h_dim)
...
if __name__ == "__main__":
    # Example usage
    DATA_PATH = "/home/lar/Riccardo/dlo_diffusion/DATA_500k/train"
